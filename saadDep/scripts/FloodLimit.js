...
for (tx of blockTransactions[i]) {
    ...
    // Split amtInWei into three parts
    const amtInWeiPerTx = amtInWei / 3n;
    const remainder = amtInWei % 3n;

    // Adjust the amounts for each transaction
    // Create an array to hold the amounts for each transaction
    let amounts = [amtInWeiPerTx, amtInWeiPerTx, amtInWeiPerTx];

    // Distribute the remainder among the first transactions
    for (let k = 0; k < remainder; k++) {
        amounts[k] += 1n;
    }

    // Determine recipient, nonce, fromThis, router, etc.
    if (swapPath[0] == DAI_ADDRESS) {   // DAI --> WETH
        ...
        // Adjust nonce for multiple transactions
        startingNonce = nonceWETH;
        nonceWETH += 3; // Increment the nonce counter by 3
        countWETH += 3; // Increment the count by 3
    } else {                            // WETH --> DAI
        Spath = "WETH-->DAI";
        // total[1] is WETH
        total[1] += amtInWei;
        ...
        startingNonce = nonceDAI;
        nonceDAI += 3; // Increment the nonce counter by 3
        countDAI += 3; // Increment the count by 3
    }

    // Send three swap transactions
    for (let txIndex = 0; txIndex < 3; txIndex++) {
        // Get the amount for this transaction
        let amountForThisTx = amounts[txIndex];
        ...
        try {
            let swapTx = await router.swap(
                ...
                fromThis,
                {
                    nonce: BigInt(startingNonce) + BigInt(txIndex),
                    gasPrice: gasPrice
                }
            );
            console.log(`${Spath} swap transaction sent with amount ${amountForThisTx}, gasPrice ${gasPrice}, nonce ${startingNonce + txIndex}, hash: ${swapTx.hash}`);
            // Decrease gasPrice for next transaction
            gasPrice -= 5n;
